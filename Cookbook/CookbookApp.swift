...
struct CookbookApp: App {
    ...
    let localContainer: ModelContainer = {
        let schema = Schema([
            RecipeGroup.self, Recipe.self, ShoppingList.self, Ingredient.self,
        ])
        let container = try! ModelContainer(for: schema, configurations: [])
        container.mainContext.autosaveEnabled = true

        let listCount = try! container.mainContext.fetchCount(
...
//                                url: URL(
//                                    string:
//                                        "https://www.halfbakedharvest.com/4-ingredient-chocolate-mousse/#wprm-recipe-container-87338"
//                                )
//                            )
//
